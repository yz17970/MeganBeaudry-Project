...
---
This document we are testing ARCO numbers and RMSE. Changed method to LM from Rpart
...
{
  fit1 <- caret::train( as.formula(paste(outcomename, "~",names(data_train)[n])) , data = data_train, method = "lm", trControl = fitControl, na.action = na.pass, tuneLength = 10)
resultmat[n-1,2]= max(fit1$results$RMSE)  
}
print(resultmat)
...
fit2 = caret::train(Pond  ~ ., data=data_train, method="lm",  trControl = fitControl, na.action = na.pass, tuneLength = 20) 
print(fit2$results)
```
