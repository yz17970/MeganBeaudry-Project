...
RASTER PLOTS
```{r}
raster_by_pond_HF183 <- ggplot(clean_data, aes(x = Date_Sampled, y = Pond, fill = HF183, interpolate = TRUE)) + geom_raster()
raster_by_pond_HF183

raster_by_pond_Entero <- ggplot(clean_data, aes(x = Date_Sampled, y = Pond, fill = Enterococcus_CCE, interpolate = TRUE)) + geom_raster()
raster_by_pond_Entero


ggplot(clean_data, aes(Date_Sampled, Pond))
```
