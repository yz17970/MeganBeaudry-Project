...
HF183_all_arranged <- grid.arrange(HF183_at_Country_Hills, HF183_at_Inverness, HF183_at_McCall)
...
#NEED TO ADD ARROWS FOR RAINFALL FOR ARCO AT MCCALL
```
...
RASTER PLOTS
```{r}
raster_by_pond_HF183 <- ggplot(clean_data, aes(x = Date_Sampled, y = Pond, fill = HF183)) + geom_raster(interpolate = TRUE) + scale_fill_gradientn(colors = c("blue", "yellow"))
raster_by_pond_HF183

raster_by_pond_Entero <- ggplot(clean_data, aes(x = Date_Sampled, y = Pond, fill = Enterococcus_CCE)) + geom_raster(interpolate = TRUE) + scale_fill_gradientn(colors = c("blue", "yellow"))
