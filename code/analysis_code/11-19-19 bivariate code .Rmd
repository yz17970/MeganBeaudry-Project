...
```{r}
arco_bar <- McCall_Lake_2 %>%
ggplot() +
  geom_bar(aes(x = arco_y_n), stat = "count", color = "yellow", fill = "white") +
  labs(x = "A. butzleri positive or negative", y = "Counts") +
  theme(panel.background = element_rect(fill = "grey85")) + 
  geom_text(stat='count', aes(label=..count..), vjust=-1)
arco_bar
ggsave(filename = "../../results/arco_bar.png", plot = arco_bar)
```
...
McCall_Acro_Graph <- ggplot(data = McCall_Lake_2, aes(x=arco_y_n)) +
  geom_bar(color = "#DDA0DD", fill = "#DDA0DD") +
  geom_text(stat='count', aes(label=..count..), vjust=-1) +
  theme(panel.background = element_rect(fill = "grey85")) +
  labs(x = title, y = "Counts")
McCall_Acro_Graph
...
```
```{r}
title <- expression(paste(italic("A. butzleri"), " positive or negative"))

ggplot(McCall_Lake_2, aes(x = arco_y_n)) +
...
```{r Temp Trend - Seagull - Inverness}
...
```{r Temp Trend - Seagull - Country Hills }
